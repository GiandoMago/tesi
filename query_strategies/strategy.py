...
class Strategy:
    ...
    def train(self, reset=True, optimizer=0, verbose=True, data=[], net=[]):
        ...
        if reset: self.clf =  self.net.apply(weight_reset).to(self.device)
    ...
    def getNet(self, params):
        i = 0
        model = deepcopy(self.clf).to(self.device)
    ...
    def get_exp_grad_embedding(self, X, Y, probs=[], model=[]):
     ...
     # Metodo che calcola la probabilità di appartenenza di un input ad una determinata classe.

    def get_prob(self, x, l):
        # Aggiunge una dimensione all'input x per conformarlo al formato richiesto dal modello
        x = x.unsqueeze(0)
        
        # Ottiene il modello di classificazione
        model = self.clf
        
        # Imposta il modello in modalità di valutazione (disabilitando l'addestramento)
        model = model.eval()
        
        # Disabilita il calcolo dei gradienti durante l'inferenza
        with torch.no_grad():
            # Esegue l'inferenza sul modello per ottenere l'output
            out, _ = model(x)
            
            # Applica la funzione di softmax all'output per ottenere le probabilità di appartenenza a ciascuna classe
            out = F.softmax(out, dim=1)
            
            # Sposta le probabilità sul dispositivo CPU e ottiene i dati
            probs = out.cpu().data
        
        # Restituisce la probabilità della classe specificata (l) dall'output
        return probs[0, l]

    # Metodo che calcola il gradiente dell'output rispetto all'input per una determinata classe

    def get_grad(self, x, l):
        # Aggiunge una dimensione all'input x per conformarlo al formato richiesto dal modello
        x = x.unsqueeze(0)
        
        # Abilita il calcolo dei gradienti per l'input x
        x.requires_grad_()
        ...
        # Azzera i gradienti del modello per evitare accumulo di gradienti
        model.zero_grad()
        
        # Calcola il gradiente dell'output rispetto all'input per la classe specificata (l)
        out[0, l].backward()
        
        # Ottiene il gradiente dal tensore di dati dell'input e crea una copia come array numpy
        gradient = x.grad.data.clone().numpy()
        
        # Restituisce il gradiente come un tensore PyTorch
